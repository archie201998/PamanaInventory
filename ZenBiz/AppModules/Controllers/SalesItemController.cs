...
namespace ZenBiz.AppModules.Controllers
{
    internal class SalesItemController : ISalesItem
    {
        ...
        public DataTable Fetch(DateTime dateFrom, DateTime dateTo)
        {
            ...
            string query = $"SELECT id, sales_id, trans_no, trans_date, customers_id, customer_name, stores_id, store_name, items_id, sku_code, item_name, unit_name, sold_unit_cost, sold_price, sold_quantity,(sold_price * sold_quantity) AS gross_sale FROM {viewSalesItem} WHERE trans_date BETWEEN @date_from AND @date_to ORDER BY trans_date DESC";
